...
extension ProjectContentViewController: MoveContentDelegate {
    ...
    func moveContentTapButton(cell: UITableViewCell, listIndex: Int) {
        ...
        for i in self.projectContent[self.currentPage].detailContent {
            ...
            self.ref.child("\(self.emailUid)/project/\(id)/content/\(currentPage)/\(currentTitle)/\(count)").setValue(["cardName": cardName, "color": color, "startTime": startTime, "endTime": endTime])
            count += 1
        }
        ...
        UserDefault().notificationModelUserDefault(title: selectCellCardName, status: "이동", content: "\"\(selectCellCardName))\" 카드가 이동되었습니다", date: koreanDate(), badge: true)
...
extension ProjectContentViewController: SendContentDelegate {
    func sendContent(_ name: String, _ index: Int, _ color: String, _ startTime: String, _ endTime: String) {
        ...
        self.ref.child("\(self.emailUid)/project/\(id)/content/\(currentPage)/\(currentTitle)/\(index)").updateChildValues(["cardName": name, "color": color, "startTime": startTime, "endTime": endTime])
